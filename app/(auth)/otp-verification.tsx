    ...
    return (
        <SafeAreaView style={styles.container}>
            ...
            <View style={styles.content}>
                <Text style={styles.subtitle}>Code has been sent to {maskedContact}</Text>
                ...
                <TouchableOpacity onPress={seconds === 0 ? handleResend : undefined} disabled={seconds > 0}>
                    <Text style={styles.resendText}>
                        {seconds > 0 ? `Resend code in  ` : 'Resend code'}
                        {seconds > 0 && <Text style={styles.timerText}>{String(seconds).padStart(2, '0')} s</Text>}
                    </Text>
                </TouchableOpacity>
            </View>

            <View style={styles.footer}>
                <Button title="Verify" onPress={() => router.push('/(auth)/create-new-password')} style={styles.verifyButton} />
...
const styles = StyleSheet.create({
    ...
    otpBox: { width: 70, height: 56, borderRadius: 12, borderWidth: 1.5, borderColor: COLORS.border, backgroundColor: COLORS.card, alignItems: 'center', justifyContent: 'center' },
