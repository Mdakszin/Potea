...
export default function AddressScreen() {
    ...
    const renderItem = ({ item }: { item: Address }) => (
        <TouchableOpacity 
            ...
            onPress={() => router.push({ pathname: '/(main)/add-address', params: { addressId: item.id, initialData: JSON.stringify(item) } })}
        >
        ...
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            {loading ? <View style={styles.centered}><ActivityIndicator size="large" color={COLORS.primary} /></View> : (
                <FlatList 
                    ...
                    ListEmptyComponent={<Text style={{ textAlign: 'center', color: colors.textLight, marginTop: 40 }}>No addresses saved yet.</Text>} 
                />
            )}
