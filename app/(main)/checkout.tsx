...
import { collection, query, where, getDocs, doc, runTransaction, serverTimestamp, onSnapshot } from 'firebase/firestore';
...
export default function CheckoutScreen() {
    ...
    useEffect(() => {
        ...
        const fetchDefaultAddress = async () => {
            try {
                ...
                if (!querySnapshot.empty) {
                    ...
                    setSelectedAddress({ id: docSnap.id, ...docSnap.data() } as any);
                }
            } catch (error) {
        ...
        fetchDefaultAddress();
    }, [currentUser, selectedAddress, setSelectedAddress]);
    ...
    const handlePlaceOrder = async () => {
        ...
        if (!payment) { Alert.alert('Error', 'Please select a payment method.'); return; }
        if (!shipping) { Alert.alert('Error', 'Please select a shipping method.'); return; }
        if (!currentUser) return;
            ...
            <ScrollView contentContainerStyle={styles.content}>
                ...
                <TouchableOpacity style={[styles.addressCard, { backgroundColor: colors.card }]} onPress={() => router.push('/(main)/shipping-address')}>
                    ...
                    <View style={styles.addressInfo}>
                        <Text style={[styles.addressLabel, { color: colors.text }]}>{address ? (address as any).label : 'Select Address'}</Text>
                        <Text style={[styles.addressText, { color: colors.textLight }]} numberOfLines={1}>{address ? (address as any).street : 'Tap to select a shipping address'}</Text>
                    </View>
                ...
                <TouchableOpacity style={[styles.shippingCard, { backgroundColor: colors.card }]} onPress={() => router.push('/(main)/choose-shipping')}>
                    <Ionicons name={(shipping as any)?.icon || 'cube-outline'} size={24} color={COLORS.primary} />
                    <View style={styles.shippingInfo}>
                        <Text style={[styles.shippingType, { color: colors.text }]}>{shipping ? (shipping as any).type : 'Select Shipping'}</Text>
                        <Text style={[styles.shippingDays, { color: colors.textLight }]}>
                            {shipping ? `Estimated Arrival, ${(shipping as any).arrival}` : 'Choose preferred method'}
                        </Text>
                    </View>
                    {shipping && <Text style={[styles.shippingPrice, { color: COLORS.primary }]}>${(shipping as any).price.toFixed(2)}</Text>}
                ...
                <TouchableOpacity style={[styles.shippingCard, { backgroundColor: colors.card }]} onPress={() => router.push('/(main)/payment-method')}>
                    <Ionicons name={(payment as any)?.icon || 'card-outline'} size={24} color={COLORS.primary} />
                    <Text style={[styles.paymentLabel, { color: colors.text }]}>{payment ? (payment as any).label : 'Select Payment'}</Text>
                ...
                <TouchableOpacity style={[styles.promoCard, { backgroundColor: colors.card }]} onPress={() => router.push('/(main)/promo-code')}>
                    ...
                    <Text style={[styles.promoLabel, { color: colors.text }]}>{promo ? `Promo: ${(promo as any).code}` : 'Enter Promo Code'}</Text>
