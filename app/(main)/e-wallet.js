    ...
    const renderTransaction = ({ item }) => (
        <TouchableOpacity style={[styles.transactionItem, { borderBottomColor: colors.border }]} onPress={() => router.push({ pathname: '/(main)/e-receipt', params: { transaction: JSON.stringify(item) } })}>
            <View style={styles.iconContainer}>
                {item.isTopUp ? (
                    <View style={[styles.topUpIcon, { backgroundColor: isDark ? 'rgba(76, 175, 80, 0.1)' : COLORS.primaryLight }]}><Ionicons name="wallet-outline" size={24} color={COLORS.primary} /></View>
                ) : (
                    <View style={[styles.spentIcon, { backgroundColor: isDark ? 'rgba(255, 76, 76, 0.1)' : 'rgba(255, 76, 76, 0.05)' }]}><Ionicons name="cart-outline" size={24} color="#FF4C4C" /></View>
                )}
            </View>
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            <View style={styles.header}>
                <View style={styles.headerLeft}><Ionicons name="leaf" size={28} color={COLORS.primary} /><Text style={[styles.headerTitle, { color: colors.text }]}>My E-Wallet</Text></View>
                <TouchableOpacity style={styles.headerBtn}><Ionicons name="search-outline" size={24} color={colors.text} /></TouchableOpacity>
            </View>
                ...
                ListHeaderComponent={
                    <View style={styles.cardContainer}>
                        <View style={[styles.card, { backgroundColor: COLORS.primary }]}>
                            <View style={styles.cardHeader}><View><Text style={styles.cardName}>{userData?.name || 'User'}</Text><Text style={styles.cardNumber}>•••• •••• •••• ••••</Text></View><Ionicons name="logo-bitcoin" size={32} color={COLORS.white} /></View>
                            <View style={styles.cardFooter}><View><Text style={styles.balanceLabel}>Your balance</Text><Text style={styles.balanceValue}>${balance.toFixed(2)}</Text></View><TouchableOpacity style={styles.topUpBtn} onPress={() => router.push('/(main)/top-up')}><Ionicons name="add-circle" size={20} color={COLORS.white} /><Text style={styles.topUpLabel}>Top Up</Text></TouchableOpacity></View>
                        </View>
                        <View style={styles.sectionHeader}><Text style={[styles.sectionTitle, { color: colors.text }]}>Transaction History</Text><TouchableOpacity onPress={() => router.push('/(main)/transaction-history')}><Text style={styles.seeAll}>See All</Text></TouchableOpacity></View>
                    </View>
                }
