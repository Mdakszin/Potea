        ...
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            {activeTab === 'faq' ? (
                <View style={{ flex: 1 }}>
                    <View style={styles.searchRow}>
                        <View style={[styles.searchBar, { backgroundColor: colors.card, borderColor: colors.border }]}>
                            ...
                            <TextInput style={[styles.searchInput, { color: colors.text }]} placeholder="Search" placeholderTextColor={colors.textLight} value={search} onChangeText={setSearch} />
                        </View>
                    </View>
                    <FlatList
                        data={filteredFaq}
                        ...
                        renderItem={({ item }) => (
                            <View style={[styles.faqItem, { backgroundColor: colors.background, borderColor: colors.border }]}>
                                ...
                                {expanded === item.id && <View style={styles.faqBody}><Text style={[styles.faqAnswer, { color: colors.textLight }]}>{item.answer}</Text></View>}
                            </View>
                        )}
                ...
                <FlatList
                    ...
                    keyExtractor={item => item.id}
                    renderItem={({ item }) => (
                        <TouchableOpacity style={[styles.contactItem, { backgroundColor: colors.background, borderColor: colors.border }]} onPress={() => item.target && router.push(item.target)}>
                            <View style={styles.contactLeft}><Ionicons name={item.icon} size={24} color={COLORS.primary} /><Text style={[styles.contactLabel, { color: colors.text }]}>{item.label}</Text></View>
                            {item.value ? <Text style={[styles.contactValue, { color: colors.textLight }]}>{item.value}</Text> : <Ionicons name="chevron-forward" size={20} color={colors.text} />}
                        </TouchableOpacity>
                    )}
                    contentContainerStyle={styles.listContent}
                />
