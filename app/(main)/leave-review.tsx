...
interface Order {
    plant?: { name: string; image: string };
    items?: OrderItem[];
    qty?: number;
    price?: number;
    total: number;
...
export default function LeaveReviewScreen() {
    ...
    const order = orderStr && typeof orderStr === 'string' ? JSON.parse(orderStr) as Order : null;
    ...
    if (!order) return <View style={[styles.container, { backgroundColor: colors.background, justifyContent: 'center', alignItems: 'center' }]}><Text style={{ color: colors.text }}>Order not found</Text></View>;

    const displayItem = order.plant || order.items?.[0];
        ...
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <View style={styles.content}>
                <View style={[styles.productCard, { backgroundColor: colors.card, borderColor: colors.border }]}>
                    <Image source={{ uri: displayItem?.image }} style={styles.productImage} />
                    <View style={styles.productInfo}>
                        <Text style={[styles.plantName, { color: colors.text }]}>{displayItem?.name}</Text>
...
const styles = StyleSheet.create({
    ...
    starContainer: { flexDirection: 'row' as const, marginBottom: 24 },
