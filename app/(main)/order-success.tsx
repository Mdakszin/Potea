...
const ConfettiDot = ({ delay, x, color, size }: ConfettiProps) => {
    ...
    return <Animated.View style={{ position: 'absolute', left: x as any, top: 0, width: size, height: size, borderRadius: size / 2, backgroundColor: color, transform: [{ translateY: animY }], opacity: animO }} />;
};
        ...
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <View style={styles.content}>
                ...
                <Text style={[styles.subtitle, { color: colors.textLight }]}>
                    Your order of ${Number(total).toFixed(2)} was successful. You'll receive an email shortly.
                </Text>
                <View style={[styles.orderIdBox, { backgroundColor: isDark ? 'rgba(76, 175, 80, 0.1)' : COLORS.primaryLight }]}>
                    ...
                    <Text style={[styles.orderId, { color: COLORS.primary }]}>{orderId as string}</Text>
                </View>
            </View>
            <View style={styles.footer}>
                <Button 
                    title="View Receipt" 
                    onPress={() => router.push({ pathname: '/(main)/e-receipt', params: { id: orderId } })} 
