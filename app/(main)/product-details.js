...
import { collection, query, where, onSnapshot, doc, updateDoc, arrayUnion, arrayRemove, setDoc, getDoc } from 'firebase/firestore';
...
export default function ProductDetailScreen() {
    ...
    if (!plant) return <View><Text>Plant not found</Text></View>;
    ...
    useEffect(() => {
        if (userData?.favorites) setIsFav(userData.favorites.includes(plant.id));
    ...
    const toggleFavorite = async () => {
        ...
        if (currentUser) {
            try {
                ...
                await updateDoc(userRef, { favorites: newFavState ? arrayUnion(plant.id) : arrayRemove(plant.id) });
            } catch (error) {
    ...
    const addToCart = async (navigate = false) => {
        ...
        try {
            ...
            if (cartDoc.exists()) {
                ...
                await setDoc(cartItemRef, { ...cartDoc.data(), qty: existingQty + quantity }, { merge: true });
            } else {
                    ...
                    <View style={styles.detailsContainer}>
                        ...
                        <View style={styles.sizeSection}>
                            ...
                            <View style={styles.sizeOptions}>
                                {plant.sizes.map(size => (
                                    <TouchableOpacity key={size} style={[styles.sizeBtn, { borderColor: colors.primary, backgroundColor: selectedSize === size ? colors.primary : 'transparent' }]} onPress={() => setSelectedSize(size)}>
                        ...
                        <View style={styles.qtySection}>
                            ...
                            <View style={[styles.qtyControl, { backgroundColor: colors.card }]}>
                                <TouchableOpacity onPress={() => setQuantity(Math.max(1, quantity - 1))}><Ionicons name="remove" size={20} color={colors.text} /></TouchableOpacity>
                                <Text style={[styles.qtyText, { color: colors.text }]}>{quantity}</Text>
                                <TouchableOpacity onPress={() => setQuantity(quantity + 1)}><Ionicons name="add" size={20} color={colors.text} /></TouchableOpacity>
                            </View>
                        </View>
                    </View>
                </ScrollView>
...
const styles = StyleSheet.create({
    ...
    detailsContainer: { padding: SPACING.lg, borderTopLeftRadius: 30, borderTopRightRadius: 30, marginTop: -30, backgroundColor: 'white' }, // Fixed background for white cards
