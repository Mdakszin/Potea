...
export default function PromoCodeScreen() {
    ...
    const [applied, setApplied] = useState<Promo | null>(appliedPromo as any);
    ...
    useEffect(() => {
        ...
        const unsubscribe = onSnapshot(q, (snapshot) => {
            const list = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Promo));
            setPromos(list);
            setLoading(false);
        });
        return () => unsubscribe();
    ...
    const handleApply = (promoToApply: Promo | null = null) => {
        ...
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <View style={styles.footer}>
                <Button 
                    ...
                    onPress={() => { setAppliedPromo(applied); router.push('/(main)/checkout'); }} 
                    disabled={!applied} 
                />
            </View>
        </SafeAreaView>
