...
export default function StripePaymentScreen() {
    ...
    const shippingFee = (shipping as any)?.price || 0;
    let promoDiscount = 0;
    if (promo) {
        const p = promo as any;
        if (p.type === 'percentage') promoDiscount = subtotal * (p.value / 100);
        else if (p.type === 'fixed') promoDiscount = p.value;
    ...
    const handlePayment = async () => {
        ...
        try {
            ...
            resetCheckout();
            router.replace({ pathname: '/(main)/order-success', params: { orderId, total: amount.toString() } });
        } catch (error: any) {
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <View style={styles.form}>
                ...
                <Button 
                    title={loading ? "Processing..." : `Pay $${Number(amount).toFixed(2)}`} 
