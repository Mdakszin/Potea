    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            ...
            <ScrollView contentContainerStyle={styles.content}>
                ...
                {METHODS.map(m => (
                    <TouchableOpacity key={m.id} style={[styles.methodItem, { backgroundColor: colors.card, borderColor: selected === m.id ? COLORS.primary : colors.border }, selected === m.id && styles.methodActive]} onPress={() => setSelected(m.id)}>
                        <View style={styles.methodLeft}><Image source={{ uri: m.icon }} style={styles.methodIcon} resizeMode="contain" /><Text style={[styles.methodLabel, { color: colors.text }]}>{m.label}</Text></View>
                        <View style={[styles.radio, { borderColor: COLORS.primary }]}>{selected === m.id && <View style={styles.radioInner} />}</View>
                    </TouchableOpacity>
                ))}
                <Button variant="outline" title="Add New Card" style={styles.addBtn} />
            </ScrollView>

            <View style={[styles.footer, { backgroundColor: colors.background, borderTopColor: colors.border }]}><Button title="Continue" onPress={() => router.push({ pathname: '/(main)/wallet-pin', params: { amount, method: selected } })} style={styles.continueBtn} /></View>
        </SafeAreaView>
    );
}
