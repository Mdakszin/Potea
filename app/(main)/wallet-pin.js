...
export default function WalletPinScreen() {
    ...
    const handleKey = (key) => { if (pin.length < PIN_LENGTH) setPin(pin + key); };
        ...
        try {
            ...
            await runTransaction(db, async (transaction) => {
                ...
                if (type === 'payment') {
                    ...
                    transaction.set(newTxRef, { name: 'Order Payment', amount, date: new Date().toLocaleDateString(), time: new Date().toLocaleTimeString(), type: 'Order', isTopUp: false, method: 'Potea E-Wallet', createdAt: serverTimestamp() });
                    const orderId = `ORD-${Date.now()}`;
                    transaction.set(doc(db, 'orders', orderId), { ...orderData, orderId, userId: currentUser.uid, paymentMethod: 'Potea E-Wallet', status: 0, createdAt: serverTimestamp(), updatedAt: serverTimestamp() });
                } else {
                    transaction.update(userRef, { balance: currentBalance + amount });
                    const newTxRef = doc(collection(db, 'users', currentUser.uid, 'transactions'));
                    transaction.set(newTxRef, { name: 'Top Up Wallet', amount, date: new Date().toLocaleDateString(), time: new Date().toLocaleTimeString(), type: 'Top Up', isTopUp: true, method: method || 'Credit Card', createdAt: serverTimestamp() });
                }
            });
            setUserData({ ...userData, balance: type === 'payment' ? (userData?.balance || 0) - amount : (userData?.balance || 0) + amount });
            setShowSuccess(true);
    ...
    return (
        <SafeAreaView style={[styles.container, { backgroundColor: colors.background }]}>
            <View style={styles.header}><TouchableOpacity onPress={() => router.back()}><Ionicons name="arrow-back" size={24} color={colors.text} /></TouchableOpacity><Text style={[styles.title, { color: colors.text }]}>{type === 'payment' ? 'Payment' : 'Top Up'}</Text><View style={{ width: 24 }} /></View>
            <View style={styles.content}><Text style={[styles.label, { color: colors.text, textAlign: 'center' }]}>Enter your PIN to confirm {type === 'payment' ? 'payment' : 'top up'}</Text>
                <View style={styles.pinWrapper}>{Array(PIN_LENGTH).fill(0).map((_, i) => (<View key={i} style={[styles.pinDigit, { borderColor: pin.length > i ? COLORS.primary : colors.border }]}><View style={[styles.pinDot, { backgroundColor: pin.length > i ? COLORS.primary : 'transparent' }]} /></View>))}</View>
                <Numpad onPressKey={handleKey} onDelete={handleDelete} colors={colors} />
                <Button title={loading ? "Processing..." : "Continue"} onPress={handleContinue} style={{ width: '100%', marginTop: 40 }} disabled={pin.length !== PIN_LENGTH || loading} />
            </View>

            <Modal visible={showSuccess} transparent animationType="fade">
                <View style={styles.modalOverlay}><View style={[styles.modalCard, { backgroundColor: colors.card }]}><View style={styles.successIcon}><Ionicons name="checkmark-sharp" size={40} color={COLORS.white} /></View><Text style={[styles.modalTitle, { color: colors.text }]}>{type === 'payment' ? 'Payment Successful!' : 'Top Up Successful!'}</Text><Text style={[styles.modalText, { color: colors.textLight }]}>{type === 'payment' ? 'You have successfully paid for your order.' : 'Your wallet has been topped up successfully.'}</Text><Button title="OK" onPress={() => { setShowSuccess(false); router.replace('/(tabs)/wallet'); }} style={{ width: '100%' }} /></View></View>
            </Modal>
        </SafeAreaView>
    );
}
